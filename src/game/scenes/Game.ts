...
export default class Game extends Phaser.Scene {
    ...
    create() {
        ...
        // Initialiser le système de combinaison d'ingrédients
        this.ingredientManager = new IngredientInteractionManager(this);
        // Ne PAS activer le drag & drop - on utilise E pour déposer et combiner

        // Placer les ingrédients sur les plans de travail pour tester les recettes
        // Recette 1 : Beurre + Farine = Pâte
        this.placeItemOnCounter(5, 2, "butter"); // Beurre en haut à gauche
        this.placeItemOnCounter(6, 2, "flour"); // Farine en haut à droite

        // Recette 2 : Chocolat + Pâte = Cookie (chocolat disponible)
        this.placeItemOnCounter(5, 3, "chocolate"); // Chocolat juste en dessous
    ...
    interactWithCounter() {
        ...
        if (this.isoMap.isCounter(this.playerGridX, this.playerGridY)) {
            ...
            targetY = this.playerGridY;
            console.log(
                `Joueur sur plan de travail, interaction sur la même position (${targetX}, ${targetY})`
            );
        }
        ...
        if (this.isoMap.isCounter(targetX, targetY)) {
            ...
            if (hasItem && this.inventory.length === 0) {
                ...
                if (itemType) {
                    ...
                    this.createCarriedItem(itemType);
            ...
            console.log(
                `Aucun plan de travail à la position (${targetX}, ${targetY})`
            );
        }
    }
