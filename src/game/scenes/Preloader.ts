...
export default class Preloader extends Phaser.Scene {
    ...
    create() {
        ...
        // Vérifier le chargement des textures de table
        console.log('🔍 Vérification des textures de table chargées:');
        const tableTextures = [
            'table-mono', 'table-open-left', 'table-open-right', 'table-open-left-top',
            'table-open-right-left', 'table-open-right-left-top', 'table-open-right-top',
            'table-open-bottom', 'table-open-left-bottom', 'table-open-right-bottom',
            'table-open-left-top-bottom', 'table-open-right-top-bottom', 'table-open-right-left-top-bottom',
            'table-open-top-bottom'
        ];
        
        tableTextures.forEach(key => {
            const texture = this.textures.get(key);
            if (texture) {
                const frame = texture.getSourceImage();
                console.log(`  ${key}: ${frame.width}x${frame.height}px`);
            } else {
                console.error(`  ❌ ${key}: NON CHARGÉE`);
            }
        });
